...
2. **Zapis przechodzi przez `build.mjs` przed commitem.** Walidator odrzucił — serwer wraca do stanu z `HEAD` (`reset`, `checkout HEAD`, `clean` po `problems/`) i nie zostawia śmiecia ani w plikach śledzonych, ani w nieśledzonych. Nie omijaj tej ścieżki. Ta sama reguła obowiązuje `plan.apply()`, nie tylko `commit()`: błąd I/O w połowie zapisu (pełny dysk, RO-mount, rozjazd praw) zostawiał nieśledzony dowód w `problems/evidence/` i blokował cały rejestr w trybie tylko do odczytu. Każda ścieżka, która dotyka dysku przed commitem, sprząta po sobie `rollback()`.
...
10. **`writes` w `/api/pulse` liczy się na ścieżce odczytu.** Flaga wyliczana wyłącznie przy zapisie kłamie w obie strony: przez całą awarię mówi `ok` (agent pali próbę, żeby się dowiedzieć), po naprawie `readonly` (agent nie próbuje wcale). Próbka (`HEAD`, brud w drzewie, blokada zapisu, liczniki, `.git/index.lock`, prawo zapisu do `.state/`) idzie na ścieżce odczytu z **sufitem raz na sekundę**, pełne `health()` dopiero po jej zmianie. Do próbki należy **każda** awaria, która zatrzymuje 100% zapisów — a więc także te dwie spoza gita i spoza drzewa: zaległy `.git/index.lock` (bierze go i commit, i sprzątanie po nim; `dirty()` i `--check` czytają z `--no-optional-locks`, więc go nie widzą) oraz `.state/` bez prawa zapisu (leżą tam blokada i oba liczniki). Powód `.git/index.lock` jest oznaczony `transient`: odczyt mówi o nim od razu, ale ścieżka zapisu **nie** odmawia natychmiast — kolizja bywa cudza i mija w ułamku sekundy, więc czeka na nią `gitReady()`. Sufit jest wymuszony, nie kosmetyczny: `execFileSync` zatrzymuje pętlę zdarzeń całego procesu, więc dwa wywołania gita na każdy odczyt dawały zmierzone 55 żądań/s przy 3400 na trasie bez gita — a `/api/pulse` jest dokładnie tą trasą, którą dokumentacja każe odpytywać. Zapis omija sufit (`guard()` wymusza pełne sprawdzenie), a granica sekundy jest tym, co ten niezmiennik obiecuje: edycja operatora jest widoczna w następnym odczycie po sekundzie, bez ani jednej próby zapisu.
...
- Limit adresu liczy się na **`/64`**, nie na adres, bo tyle dostaje jeden klient IPv6 — a `ipKey()` musi najpierw rozwinąć `::` do ośmiu grup. Cięcie napisu po `:` bierze cztery pierwsze pola *napisu*, a nie cztery pierwsze grupy *adresu*: `2001:db8::1` i `2001:db8::2` lądowały wtedy w osobnych kubełkach (tak samo dwa zapisy tego samego adresu), czyli cap był darmowy. Adres, którego nie da się rozwinąć, liczy się po całym napisie — strona ostrożna, nigdy przepustka.
- **Awaria nośnika to `503` z `reason` i `fix`, nigdy `500` z samym `ref`.** `500` mówi agentowi „nie wiem", a operatorowi każe iść do `journalctl`; `EACCES`/`ENOSPC`/`EROFS` (zbiór `IO_ERR`) to stan środowiska, o którym potrafimy powiedzieć wprost, co i jaką komendą naprawić. Ta sama reguła dotyczy zapisów do `.state/` (`writeState`) i nieudanego `plan.apply()`.
