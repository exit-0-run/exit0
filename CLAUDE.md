...
14. **A ref is claimable only under its own key's fingerprint, and solution code still never enters `main`.** An attempt with nowhere of its own to live is a ref inside a repository: `refs/attempts/<problem>/<fingerprint>/<slug>`. The grammar is checked in `sign.mjs` (`REF_RE`) and the claim inside it in the server: the problem segment must match the submission (400) and the fingerprint segment must be the submitter's own (403), compared against `fingerprint(key)` and never against a string from the body. `refs/heads` and `refs/tags` are outside the grammar, and the reason is NOT the one first written here ("so publishing cannot become writing to a served branch" - a prefix rule would give that too). It is measured. **A normal `git clone` fetches `refs/tags/*` and does not fetch `refs/attempts/*`.** Put attempts on tags and every clone of the registry drags down every attempt's code: "there is no solution code here" stops being true, the clone stops being small, and a clone that fetched no attempt refs stops being a thing that can exist. Branches lose differently: a branch can be opened as a pull request into `main`, so "solution code never in `main`" would rest on a maintainer declining rather than on the host being unable to offer it. The cost of the namespace is real and is paid by humans: no web UI lists these refs, and a commit reached by sha shows "this commit does not belong to any branch". That is why `/work` and the problem page print `git fetch <repo> <ref> && git checkout FETCH_HEAD` whenever an entry carries one. `ref` is signed, it enters `solutionId`, and the chain key is **`(problem, repo, ref, key)`** in BOTH the server and `build.mjs`: hosted attempts share one repo URL by design, so a dedup that stops at `(repo, key)` calls the second attempt a duplicate and refuses a write the server was right to accept. That was a real 422 during development, not a hypothetical. `main` still carries problems, evidence and scripts and no solution code; a clone that fetched no attempt refs still passes `--check`.
