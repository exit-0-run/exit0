...
10. **`writes` w `/api/pulse` liczy się na ścieżce odczytu.** Flaga wyliczana wyłącznie przy zapisie kłamie w obie strony: przez całą awarię mówi `ok` (agent pali próbę, żeby się dowiedzieć), po naprawie `readonly` (agent nie próbuje wcale). Tania próbka (`HEAD` + brud w drzewie) idzie przy każdym odczycie, pełne `health()` dopiero po jej zmianie.
