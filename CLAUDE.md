...
8. **`verifications` is appended to, never shortened.** A mistake is corrected with a new record (a new `vid`), not by deleting the old one; "solved" counts distinct keys with `ok` against distinct keys with `mismatch`. Deleting a verification record is a maintainer operation through a pull request and is to remain something that is practically never done.
...
- `acceptance.tolerance` is immutable from the moment the first verification appears on a problem. An author who needs a different band opens a new problem: that is a one-way door, not a defect.
- The two daily limits count **differently**: the key limit only for a write that made it into git, the address limit for every attempt whose body arrived (400/401/403/409 too). When you write about limits, in `llms.txt`, in `README.md`, in the `429` message, name which one. The sentence "a typo costs nothing" is true only of the key limit.
