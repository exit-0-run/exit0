...
It never empties, either. A result confirmed by one key has been run once, on one machine,
so it stays in the queue as `SECOND RUN` until a second stranger has run it. That verdict
changes no status and cannot: it changes what the number is worth. Every view says how many
